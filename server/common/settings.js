...
exports.loadSettings = async() => {
    ...
    let tmp = [];
    // Browsers
    let browsers = await Browsers.findAll({ order: [['id', 'ASC']], attributes: ['id', 'name'] });
    ...
    // Country
    let countries = await Countries.findAll({ order:[['id', 'ASC']], attributes: ['id', 'code', 'name', 'dial_code'] });
    countries.forEach(data => {
        tmp[data.dataValues.id] = [data.dataValues.code, data.dataValues.name, data.dataValues.dial_code];
    });
    App_Settings.countries = {...tmp};
    tmp = [];

    // Devices
    let devices = await Devices.findAll({ order:[['id', 'ASC']], attributes: ['id', 'name'] });
    ...
    // Languages
    let languages = await Languages.findAll({ order:[['id', 'ASC']], attributes: ['id', 'name'] });
    ...
    // Categories
    let categories = await Categories.findAll({ order:[['id', 'ASC']], attributes: ['id', 'name'] });
    ...
    // Os
    let os = await Os.findAll({ order:[['id', 'ASC']], attributes: ['id', 'name', 'version'] });
    os.forEach(data => {
